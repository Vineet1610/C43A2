...
--Views (Query 10)
CREATE VIEW greaterWinsThanLosses AS 
(select * from 
    (select * from record where year = 2014) 
 where wins > losses);
 
CREATE VIEW playerSubset as select pid from greaterWinsThanLosses;
CREATE VIEW playerDurations as select pid, duration from playerSubset cross join event where pid = winid or pid = lossid;
CREATE VIEW averageDuration as select pid, avg(duration) as avgDuration from playerDurations group by pid;
CREATE VIEW greaterThan200Duration as select pid from averageDuration where avgDuration > 200;
CREATE VIEW qualifiedPIDs as select pid from ((select pid from playerSubset) intersect (select pid from greaterThan200Duration));
CREATE VIEW qualifiedPlayerNames as select pname from qualifiedPIDs natural join player;

--Query 10
INSERT INTO query10(select pname FROM qualifiedPlayerNames order by pname desc);

--Drop Views
DROP VIEW playerMatchesIn2014 CASCADE;
